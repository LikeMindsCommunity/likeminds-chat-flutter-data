...
import 'package:likeminds_chat_fl/src/persistence/logger/utils/severity_level_utils.dart';
...
class LMChatLogger {
  ...
  LMChatLogger._internal();

  static LMChatLogger? _instance;

  static LMChatLogger get instance => _instance ??= LMChatLogger._internal();
  ...
  // of LogDBHandler
  Future<void> _insertLogs(LMStackTrace stackTrace, LMSeverity severity) async {
    ...
    LMSDKMeta lmSdkMeta = (LMSDKMetaBuilder()
          ..dataLayerVersion(chatSDKVersion)
  ...
  Future<LMResponse<void>> _pushLogs() async {
    if (!checkIfLoggerInitialised()) {
      return LMResponse(
          success: false, errorMessage: 'LMChatLogger not initialised');
    }
    int currentTimeStamp = DateTime.now().millisecondsSinceEpoch;
    ...
    LMDeviceDetailsBuilder deviceDetailsBuilder = LMDeviceDetailsBuilder();
    ...
    if (Platform.isAndroid) {
      ...
      deviceDetailsBuilder..wifi(isOnWifi);
    ...
    LMDeviceDetails deviceMeta = deviceDetailsBuilder.build();
  ...
  void handleException(Exception exception, StackTrace stackTrace,
    ...
    // logLevel set by the LMChatClient, then do nothing
    if (!checkLMSeverityForLoggingErros(
        errorLMSeverity: errorSeverity,
